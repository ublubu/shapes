...
toTileCoord :: RealFrac a => Transform (Pair a) -> Pair a -> Pair Int
toTileCoord (Transform scale origin) click = fmap floor ((/) <$> (click - origin) <*> scale)

clickTile :: (RealFrac a, Draggable t) =>  Transform (Pair a) -> Pair a -> t -> Maybe t
...
tileRect :: Num a => Transform (Pair a) -> Pair Int -> Rectangular a
...
tileBoundingRect :: Num a => Transform (Pair a) -> Pair Int -> Rectangular a
tileBoundingRect drawInfo coord = (+) <$> signedRect <*> tileRect drawInfo coord

toBoundingRect :: (Ord a, RealFrac a) => Transform (Pair a) -> Pair Int -> Pair a -> GridDirection -> Rectangular a
...
dragResult :: (Ord a, RealFrac a, Draggable b) => Transform (Pair a) -> Pair (Pair a) -> b -> DragResult a b
...
completelyApplyDrag :: (Ord a, RealFrac a, Draggable b) => Transform (Pair a) -> Pair (Pair a) -> b -> (Pair (Pair a), Maybe (PartialMoveResult a), b)
...
shouldConvert :: (Ord a, RealFrac a) => Transform (Pair a) -> GridOriented a -> Bool
...
convertPartialMove :: (Ord a, RealFrac a) => Transform (Pair a) -> PartialMoveResult a -> Maybe FullMoveResult
