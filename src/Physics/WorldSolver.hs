...
wsolve :: WSGen w k a x s -> WSFunc w k a s -> WSChanged w -> Int -> [k] -> WorldLens k w a -> w -> x -> s -> (w, s)
wsolve g f changed n ks l w x s = wsimprove f changed n l w s1
  where s1 = g ks l w x s

wsolve' = uncurry wsolve
