...
module Physics.Solver.Simple.Contact where
...
newtype ConstraintGen' n a =
  ConstraintGen' { _cgen' :: n
                          -> Key
                          -> (a, a)
                          -> ([(Key, ConstraintGen n a)], ConstraintGen' n a)
                 }
...
updatePairCache featurePairCgens emptySolution (Just featurePairCaches0) =
  ...
  where f !featurePairCaches (!pairKey, !cgen) =
          ...
          where sln' = fromMaybe emptySolution (fst <$> lookupPair pairKey featurePairCaches0)
...
worldCacheInitializer :: CS.WorldCacheInitializer World a n (WorldCache n a)
